...
#define MEMORY_NODE_NUM 5        // [MODIFIED BY sed_MN_num]
#define CPU_PHYSICAL_CORE_NUM 48 // [CONFIG]  72
...
// app thread
#define MAX_APP_THREAD 49 // one additional thread for data statistics(main thread)  [CONFIG] 65
...
{
  // KV size
  constexpr uint32_t keyLen = 8;          // [MODIFIED BY sed_key_len]
  constexpr uint32_t simulatedValLen = 8; // [MODIFIED BY sed_val_len]
...
{ // namespace define
  ...
  // Cache (MB)
  constexpr int kIndexCacheSize = 100; // [MODIFIED BY sed_cache_size]
  constexpr int kHotspotBufSize = 30;  // [MODIFIED BY sed_hotspot_buffer_size]
  ...
  // Leaf Node
  constexpr uint32_t leafSpanSize = 64; // [MODIFIED BY sed_span_size]
  ...
  // Hopscotch Hashing
  constexpr uint32_t neighborSize = 8; // [MODIFIED BY sed_neighbor_size]
...
// Rdma Read/Write Size
  ...
  // Greedy
  constexpr uint64_t greedySizePerIO = transLeafSize / 2; // [MODIFIED BY sed_greedy_IO_size]
